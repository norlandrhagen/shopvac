...
async def get_prefix_size_with_progress(
    ...
    try:
        ...
        progress.update(task_id, description=f"Failed {prefix.rstrip('/')}: {type(e).__name__}")
        raise
    ...
    **provider_options
    ...
    if show_progress:
        with Progress(
            ...
            tasks = [
                get_prefix_size_with_progress(
                    store, prefix, progress, 
                    progress.add_task(f"Waiting {prefix.rstrip('/')}", total=None),
                    timeout_per_prefix
                )
    ...
    filtered_data = [
        (prefix, size) for prefix, size in results 
